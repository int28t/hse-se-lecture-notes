...
По уровню видимости в базовой программе обычно встречаются:
\begin{itemize}
    \item \textbf{namespace scope} (в т.ч. глобальная область);
    \item \textbf{block scope} внутри функций и управляющих конструкций.
\end{itemize}

Связь с инструментами сборки прямая: директивы обрабатывает препроцессор, синтаксис и типы — компилятор, связывание внешних определений — линкер.
...
\begin{itemize}
    ...
    \item для управления памятью: \texttt{new}, \texttt{delete};
    \item для исключений: \texttt{try}, \texttt{catch}, \texttt{throw};
    \item для управления доступом: \texttt{public}, \texttt{private}, \texttt{protected};
\end{itemize}
...
Примеры:
\begin{itemize}
    \item корректные: \texttt{value}, \texttt{myVar2}, \texttt{\_tmp};
    \item некорректные: \texttt{2value}, \texttt{my-var}, \texttt{class}.
\end{itemize}

С учётом code style обычно придерживаются такой схемы:
\begin{itemize}
    \item объекты и функции — \texttt{camelCase};
    \item типы — \texttt{PascalCase};
    \item глобальные константы — \texttt{kPascalCase} или \texttt{UPPER\_CASE};
    \item макросы препроцессора — \texttt{UPPER\_CASE}.
\end{itemize}
...
\end{verbatim}
\begin{verbatim}
const double pi = 3.14159; // definition
\end{verbatim}
\end{definition}
...
Для функций действует та же идея:
\begin{itemize}
    \item может быть только описание (прототип), а определение — в другом модуле трансляции;
    \item определение функции одновременно является и её описанием.
\end{itemize}
...
\begin{remark}
\texttt{\{\}} часто называют унифицированным инициализатором. Его обычно рекомендуют по умолчанию, когда важно получить единый стиль и защиту от сужающих преобразований.
\end{remark}
...
Такой блок можно использовать и без заголовка \texttt{if/for/while}: это удобно, чтобы ограничить время жизни временных объектов и не «протекать» именами дальше по коду.
...
В C++ объект без типа существовать не может: тип обязателен, потому что именно он задаёт размер, допустимые операции и правила интерпретации памяти.
...
Типичные модификаторы для производных типов в учебной трёхтаксонной схеме:
\begin{itemize}
    \item указатель: \texttt{T*};
    \item ссылка: \texttt{T\&}, \texttt{T\&\&};
    \item массив: \texttt{T[N]};
    \item функция: \texttt{R(Args...)};
    \item указатель на член класса: \texttt{T C::*}.
\end{itemize}
...
Выражение может состоять из одного операнда (например, \texttt{x}). Из одного оператора без операндов выражение составить нельзя.
...
\subsection{Пример дерева разбора выражения}
Для выражения
\[
1 + 2 * 3
\]
дерево разбора строится с учётом приоритета операторов: сначала связывается \texttt{2 * 3}, потом выполняется сложение с \texttt{1}. Скобки \texttt{(1 + 2) * 3} меняют дерево разбора и, как следствие, результат вычисления.

\subsection{Приоритет и ассоциативность операторов}
Приоритет оператора показывает, какие операции связываются раньше при разборе выражения. Если у нескольких операторов одинаковый приоритет, используется ассоциативность: слева направо или справа налево.

\begin{itemize}
    \item приоритет определяет структуру дерева разбора;
    \item скобки всегда сильнее приоритета;
    \item ассоциативность нужна только тогда, когда приоритет одинаковый.
\end{itemize}

\begin{remark}
Например, в \texttt{a - b - c} вычитание обычно левoассоциативно: выражение читается как \texttt{(a - b) - c}. У присваивания \texttt{=} ассоциативность обычно правая.
\end{remark}
...
\texttt{break} в \texttt{case} обычно обязателен, иначе произойдёт проваливание в следующую ветку. Намеренное проваливание допустимо, но его надо явно отмечать (например, \texttt{[[fallthrough]];}) и использовать осознанно.

\texttt{switch} с инициализацией (C++17):
\begin{verbatim}
switch (int x = f(); x) {
case 1:
    ...
...
Ключевое отличие: тернарный оператор — это \textbf{выражение}. Его можно использовать там, где нужен результат выражения (инициализация, аргументы вызова, \texttt{return}), тогда как \texttt{if} — это инструкция.
...
Требования к перечисляемой коллекции: должна существовать пара границ диапазона (через \texttt{begin/end} или совместимый ADL-интерфейс), а элементы должны быть доступны через разыменование итератора.
