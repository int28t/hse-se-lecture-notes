...
\begin{itemize}
    \item \textbf{input iterators};
    \item \textbf{output iterators};
...
\begin{itemize}
    \item input — чтение последовательности вперёд;
    \item output — запись последовательности вперёд;
...
\begin{remark}
Категории чтения/записи и категории перемещения частично ортогональны, а по «силе перемещения» действует вложенность: random access сильнее bidirectional, а bidirectional сильнее forward.
\end{remark}
...
Четыре типичных сценария из контрольных вопросов:
\begin{itemize}
    \item от первого к запоследнему: \texttt{it = begin(); ++it ... it == end()};
    \item после достижения \texttt{end()} вернуться к первому: просто присвоить \texttt{it = begin()};
    \item от последнего к передпервому: \texttt{it = rbegin(); ++it ... it == rend()} или через \texttt{--} для bidirectional;
    \item после достижения начала в обратном обходе вернуться к последнему: \texttt{it = rbegin()} (или \texttt{it = --end()} для bidirectional).
\end{itemize}
...
\subsection{Когда итераторы остаются валидными, а когда нет?}
Правило зависит от контейнера и операции.

\begin{itemize}
    \item Для \texttt{std::vector} вставка или удаление в середине сдвигает элементы, а при переаллоцировании инвалидируются все итераторы, ссылки и указатели на элементы.
    \item Для \texttt{std::list} и похожих связных структур удаление инвалидирует обычно только итератор на удалённый элемент, остальные остаются валидными.
    \item Для \texttt{std::set} и \texttt{std::map} вставка обычно не инвалидирует существующие итераторы, а удаление инвалидирует только итератор на удалённый элемент.
\end{itemize}

\begin{remark}
Если алгоритм хранит итератор на элемент контейнера, нужно знать правила инвалидирования именно для этого контейнера. Иначе можно продолжить работу с уже невалидной позицией.
\end{remark}
