...
\begin{theorem}
    ...
    \[
        H = L(a_1,\ldots,a_k),
    \]
    где $a_1,\ldots,a_k$ --- л.н.з. Тогда $\forall x \in E$:
    \[  h
        =
        \operatorname{Pr}_{H}x
        =
        A(A^TA)^{-1}A^Tx,
    \]
    где
    \[
        A = (a_1,\ldots,a_k)
    \]
    --- матрица $n \times k$, составленная из столбцов $a_1,\ldots,a_k$ в ОНБ.
\end{theorem}
...
\begin{remark}
    ...
    \[
        \text{частное решение НСЛАУ}
        +
        \text{общее решение ОСЛАУ}.
    \]
...
\begin{theorem}
    ...
    \[
        P = x_0 + L(a_1,\ldots,a_k),
    \]
    а $a_1,\ldots,a_k$ --- л.н.з. векторы, может быть найдено по формуле:
    \[
        \rho(P,M)
        =
        \sqrt{
            \frac{
                Gr(a_1,\ldots,a_k, x-x_0)
            }{
                Gr(a_1,\ldots, a_k)
            }
        }.
...
\begin{definition}
    Линейный оператор $A^*: E \to E$ называется сопряженным к линейному оператору
    \[
        A: E \to E,
    \]
    если
...
\begin{theorem}
    ...
    \]
    где $\Gamma$ --- матрица Грама базиса $b$.
\end{theorem}
...
\begin{theorem}
    Собственые векторы самосопряженного линейного оператора, отвечающие различным собственным значениям, ортогональны.
\end{theorem}
...
\begin{definition}
    ...
    \]
\begin{center}
    \includegraphics[width=0.7\textwidth]{2xvzU5gekHR9xUkpnS2_3ZX7GAI7VK-J0NFHh9OdcidtbiU8SWBUbxvBaEdc9iXbjfMtn2vqyoULQqwqLEHLWnPV.jpg}
\end{center}
...
\begin{definition}
    Линейный оператор $A: E \to E$ называется ортогональным, если:
    \[
        \forall x, чy \in E:\quad (Ax, Ay) = (x, y).
    \]
...
\begin{theorem}[О спектральном разложении]
    ...
    \[
        ...
        \end{pmatrix}
    \]
    --- диагональная матрица с с.з. $\lambda_i$ оператора с матрицей $A$,
    повторяющимися соответственно их кратности.
\end{theorem}
...
\begin{theorem}
    ...
    Пусть $A_{\varepsilon_1\varepsilon_2}$ --- матрица линейного отображения $\varphi$ в паре базисов
    $\varepsilon_1$ в $V_1$ и $\varepsilon_2$ в $V_2$.
    
    Если $T_1$ --- матрица перехода от базиса $\varepsilon_1$ к базису $\varepsilon_1'$ в $V_1$,
    а $T_2$ --- матрица перехода от базиса $\varepsilon_2$ к базису $\varepsilon_2'$ в $V_2$, то
    \[
        A_{\varepsilon_1'\varepsilon_2'}
        =
        T_2^{-1}A_{\varepsilon_1\varepsilon_2}T_1.
    \]
\end{theorem}

\begin{remark}
    В случае линейного оператора, то есть когда
    \[
        \varphi: V \to V,
    \]
    и меняется один и тот же базис, формула имеет вид:
    \[
        A' = T^{-1}AT.
    \]
\end{remark}
...
\begin{Proof}
    Докажем формулу для линейного оператора.
    ...
    Пусть $A$ --- матрица оператора $\varphi$ в старом базисе, а $A'$ --- матрица этого же оператора в новом базисе.
    ...
    \[
        x'
    \]
    --- столбец координат этого же вектора в новом базисе.
    
    Аналогично:
    \[
        y
    \]
    --- координаты вектора $\varphi(x)$ в старом базисе,
    \[
        y'
    \]
    --- координаты вектора $\varphi(x)$ в новом базисе.
    
    В старом базисе действие оператора записывается так:
    \[
        y = Ax.
    \]
    
    При замене базиса координаты меняются по формуле:
    \[
        x = Tx',
    \]
    где $T$ --- матрица перехода от старого базиса к новому.
    
    Для вектора $y$ аналогично:
    \[
        y = Ty'.
    \]
    
    Подставим эти равенства в формулу
    \[
        y = Ax.
    \]
    ...
    Так как $T$ --- матрица перехода между базисами, она невырожденная. Значит, существует
    \[
        T^{-1}.
    \]
    
    Умножим равенство слева на $T^{-1}$:
    \[
        T^{-1}Ty' = T^{-1}ATx'.
    \]
    ...
    Но в новом базисе действие оператора записывается как
    \[
        y' = A'x'.
    \]
    ...
    Так как это верно для любого вектора $x'$, получаем:
    \[
        A' = T^{-1}AT.
    \]
\end{Proof}
...
\begin{Proof}
    Это формула верна, так как $\Gamma$ --- матрица билинейной формы
    \[
        g(x,y).
    \]
    
    Поэтому при замене базиса она преобразуется по формуле:
    \[
        \Gamma' = U^T \Gamma U.
    \]
\end{Proof}
...
\begin{Proof}
    ...
    По формуле преобразования матрицы Грама:
    \[
        \Gamma' = U^T \Gamma U.
    \]
    ...
    \[
        ...
        \det(U^T\Gamma U)
        =
        \det U^T \cdot \det \Gamma \cdot \det U.
    \]
    ...
    \[
        \det \Gamma'
        =
        (\det U)^2 \cdot \det \Gamma.
    \]
    
    Перейдём к ОНБ. В ОНБ
    \[
        \Gamma' = E,
    \]
    поэтому
...
\begin{Proof}
    ...
    Пусть в результате ортогонализации Грама--Шмидта из базиса
    \[
        a_1,\ldots,a_n
    \]
    получен ортогональный базис
    \[
        b_1,\ldots,b_n.
    \]
    
    Матрица перехода от базиса $a_1,\ldots,a_n$ к базису $b_1,\ldots,b_n$ имеет вид
    \[
        U_{a\to b}
        =
        \begin{pmatrix}
...
\begin{theorem}
    Векторы $a_1,\ldots,a_k \in E$ линейно независимы
    \[
        \Longleftrightarrow
        Gr(a_1,\ldots,a_k) \neq 0.
    \]
...
\begin{Proof}
    ...
    \[
        \alpha_1 a_1 + \ldots + \alpha_k a_k = 0.
    \]

    Умножим скалярно на каждый вектор $a_j$, $j=1,\ldots,k$:
    \[
        \begin{cases}
            \alpha_1 (a_1,a_1) + \alpha_2 (a_2,a_1) + \ldots + \alpha_k(a_k,a_1) = 0\\
            \vdots\\
            \alpha_1 (a_1,a_k) + \alpha_2 (a_2,a_k) + \ldots + \alpha_k(a_k,a_k) = 0
        \end{cases}
    \]

    Это однородная СЛАУ для коэффициентов $\alpha_1,\ldots,\alpha_k$ с матрицей Грама:
    \[
        \Gamma(a_1,\ldots,a_k) \alpha = 0.
    \]

    Следовательно:
    \begin{itemize}
        \item Если $Gr(a_1,\ldots,a_k) = \det \Gamma \neq 0$, то СЛАУ имеет только тривиальное решение $\alpha = 0$ $\Rightarrow$ векторы линейно независимы.
        \item Если $Gr(a_1,\ldots,a_k) = 0$, то существует ненулевое решение $\alpha \neq 0$ $\Rightarrow$ векторы линейно зависимы.
    \end{itemize}
\end{Proof}
\subsection{Докажите теорему о том, что евклидово пространство можно представить в виде прямой суммы подпространства и его ортогонального дополнения.}

\begin{theorem}
    Ортогональное дополнение $H^{\perp}$ является линейным подпространством в $V$ и
    \[
        V = H \oplus H^{\perp}
    \]
    \[
        \dim V = \dim H + \dim H^{\perp}.
    \]
\end{theorem}

\begin{Proof}
    $H^{\perp}$ является подпространством, т.к. замкнуто относительно операции сложения и умножения на число.
    
    Пусть
    \[
        h \in H,\quad x,y \in H^{\perp},\quad \alpha \in \mathbb{R}.
    \]
    ...
    Также
    \[
        (\alpha x,h) = \alpha(x,h) = \alpha \cdot 0 = 0
        \Rightarrow
        \alpha x \in H^{\perp}.
    \]
    
    Т.к. $H^{\perp}$ является подпространством, то можно рассматривать
    \[
        H + H^{\perp}.
    \]
    
    Осталось показать, что сумма прямая и что
    \[
        V = H + H^{\perp}.
    \]
    ...
    Дополним его до базиса в $V$ векторами
    \[
        f_{m+1},\ldots,f_n.
    \]
    ...
    \[
        ...
        \longrightarrow
        f_1,\ldots,f_m,e_{m+1},\ldots,e_n.
    \]
    ...
    \[
        f_1,\ldots,f_m,
    \]
    то есть они ортогональны всему $H$.
    
    И $\forall x \in V$ можно представить в виде:
    \[
        x =
        \underbrace{x_1f_1+\ldots+x_mf_m}_{\in H}
        +
        \underbrace{x_{m+1}e_{m+1}+\ldots+x_ne_n}_{\in H^{\perp}}.
    \]
    
    Т.е.
    \[
        \forall x \in V\quad x = y+z,
        \qquad
        y \in H,\ z \in H^{\perp}.
    \]
    
    Но это и означает, что всё пространство $V$ равно
    \[
        H \oplus H^{\perp}.
    \]
\end{Proof}
\subsection{Выпишите формулу ортогональной проекции вектора на её подпространство, заданное как линейная оболочка данного линейно независимого набора векторов, и докажите её.}
...
\begin{theorem}
    ...
    \[
        H = L(a_1,\ldots,a_k),
    \]
    где $a_1,\ldots,a_k$ --- линейно независимые векторы. Тогда ортогональная проекция $y = \operatorname{Pr}_H x$ имеет вид:
    \[
        y = A(A^T A)^{-1}A^T x,
    \]
    где $A$ составлена из столбцов координат $a_1,\ldots,a_k$ в ОНБ.
\end{theorem}

\begin{Proof}
    Пусть
    \[
        y = \operatorname{Pr}_H x = \alpha_1 a_1 + \ldots + \alpha_k a_k \in H,
    \]
    а
    \[
        h \in H^\perp
    ...
    Последовательно умножим $x$ на $a_1,\ldots,a_k$:
    \[
        \begin{cases}
            \alpha_1 (a_1,a_1) + \alpha_2 (a_2,a_1) + \ldots + \alpha_k(a_k,a_1) = (a_1,x)\\
            \vdots\\
            \alpha_1 (a_1,a_k) + \ldots + \alpha_k(a_k,a_k) = (a_k,x)
        \end{cases}
    \]

    В матричной форме:
    \[
        \Gamma_{xx}(a_1,\ldots,a_k)\cdot \alpha = A^T x \Longleftrightarrow A^T A \alpha = A^T x,
    \]
    где $A = [a_1,\ldots,a_k]$.

    Так как $a_1,\ldots,a_k$ л.н.з., матрица $\Gamma$ невырождена, значит
    \[
        \alpha = (A^T A)^{-1} A^T x.
    \]
    ...
    Следовательно:
    \[
        y = \sum_{i=1}^k \alpha_i a_i = A(A^T A)^{-1} A^T x.
    \]
\end{Proof}
...
\begin{Proof}
    Покажем, что л.о. с матрицей
    \[
        B = \Gamma^{-1}A_b^T\Gamma
    \]
    является сопряжённым к данному л.о.

    Проверим выполнение равенства:
    \[
        \forall x,y \in E:\quad (Ax,y) = (x,By).
    \]
    ...
    \[
        x^b,\ y^b
    \]
    --- столбцы координат векторов $x$ и $y$ в базисе $b$.
    ...
    \[
        (x,y) = (x^b)^T\Gamma y^b
    \]
    --- матричная запись скалярного произведения.
    ...
    Раскрываем транспонирование:
    \[
        (A_bx^b)^T\Gamma y^b
        =
        (x^b)^T A_b^T \Gamma y^b.
    \]
    ...
    Так как $\Gamma$ невырождена, умножим слева на $\Gamma^{-1}$:
    \[
        B = \Gamma^{-1}A_b^T\Gamma.
    \]
    ...
    Единственность следует из того, что матрица $B$ однозначно определяется равенством
    \[
        \Gamma B = A_b^T\Gamma.
    \]
\end{Proof}
\subsection{Свойство собственных векторов самосопряженного оператора, отвечающих разным собственным значениям.}

\begin{theorem}
    С.в. самосопряженного л.о., отвечающие различным с.з., ортогональны.
\end{theorem}

\begin{Proof}
    Пусть
    \[
        Ax_1 = \lambda_1 x_1,\quad x_1 \neq 0,\qquad
        Ax_2 = \lambda_2 x_2,\quad x_2 \neq 0,\qquad
        \lambda_1 \neq \lambda_2
    \]
    (т.е. $x_1,x_2$ --- с.в., соответствующие с.з. $\lambda_1, \lambda_2$).

    Так как $A$ --- самосопряженный оператор:
    \[
        (Ax_1,x_2) = (x_1,Ax_2)
    \]
    ...
    Тогда
    \[
        \lambda_1 (x_1,x_2) = \lambda_2 (x_1,x_2) \quad \Rightarrow \quad (\lambda_1-\lambda_2)(x_1,x_2) = 0
    \]

    Так как $\lambda_1 \neq \lambda_2$, получаем:
    \[
        (x_1,x_2) = 0.
    \]

    Следовательно, $x_1 \perp x_2$.
\end{Proof}
\subsection{Каким свойством обладают собственные значения самосопряженного оператора?}
...
\begin{Proof}
    Пусть $\tilde{\lambda} \in \mathbb{C}$ --- корень характеристического уравнения
    \[
        \chi_A(\lambda) = 0,
    \]
    т.е.
    \[
        \det(A - \tilde{\lambda} E) = 0.
    \]

    Тогда однородная СЛАУ
    \[
        (A - \tilde{\lambda} E)x = 0
    \]
    имеет ненулевое решение
    \[
        x_k \in \mathbb{C},\ k=1,\ldots,n.
    \]
    
    Рассмотрим столбец сопряжённых элементов:
    \[
        \bar{x} =
        \begin{pmatrix}
            \bar{x}_1\\
    ...
    Умножим слева на $\bar{x}^T$:
    \[
        \bar{x}^T (A - \tilde{\lambda} E)x = 0
        \Longleftrightarrow
        \bar{x}^T A x - \tilde{\lambda} \bar{x}^T x = 0.
    \]

    Пусть
    \[
        z = \frac{\bar{x}^T A x}{\bar{x}^T x} \quad \text{(отношение Релея)}.
    \]

    Тогда $\tilde{\lambda} = z$. Осталось показать, что $z \in \mathbb{R}$.

    Так как $A$ самосопряжённая матрица (действительная симметрическая):
    \[
        z = \frac{\bar{x}^T A x}{\bar{x}^T x}
        = \frac{\bar{x}^T A x}{\bar{x}^T x}^T
        = \frac{\bar{x}^T A x}{\bar{x}^T x}
        = \frac{x^T A x}{x^T x} \in \mathbb{R}.
    \]

    Следовательно,
    \[
        \tilde{\lambda} \in \mathbb{R}.
    \]
\end{Proof}
\subsection{Сформулируйте теорему о существовании для самосопряженного оператора базиса из собственных векторов.}
...
\begin{theorem}
    ...
    \[
        \lambda_1, \ldots, \lambda_n
    \]
    самосопряжённого л.о.
    \[
        A: E \to E
    \]
    попарно различны, то в $E$ существует ОНБ, в котором матрица оператора $A$ диагональна.
\end{theorem}

\begin{Proof}
    Так как $\lambda_1,\ldots,\lambda_n$ попарно различны, выбрав для каждого $\lambda_i$ соответствующий с.в., получаем систему ненулевых векторов.
    
    По утверждению о ортогональности с.в., соответствующих различным с.з., эта система будет ортогональна. Она л.н.з. и содержит $n$ векторов.
    
    Следовательно, это базис в $E$ ($\dim E=n$). Это ортогональный базис.
    
    \bigskip
    
    После нормировки векторы остаются собственными, поэтому полученный ортонормированный базис также состоит из собственных векторов.
    
    Далее по критерию диагональности матрицы оператора матрица оператора диагональна.
\end{Proof}
\subsection{Сформулируйте и докажите критерий ортогональности оператора, использующий его матрицу.}

\begin{theorem}
    Матрица л.о. $A$ в ОНБ ортогональна
    \[
        \Longleftrightarrow
    \]
    $A$ --- ортогональный линейный оператор.
...
\begin{Proof}
    ...
    Нужно доказать: $A$ --- ортогональный линейный оператор.

    Пусть $x, y \in E$, тогда их координаты в базисе $e$:
    \[
        x \mapsto
    ...
    Тогда
    \[
        (Ax, Ay) = (A_e x)^T \cdot E \cdot (A_e y) = (x_1\ \ldots x_n)(A_e^T A_e)(y_1\ \ldots y_n)^T = x^T y = (x,y)
    \]

    Следовательно, $A$ сохраняет скалярное произведение, значит $A$ --- ортогональный оператор.

    \bigskip

    {\bf Достаточность.}

    Дано: $A$ --- ортогональный линейный оператор.
    ...
    В координатах любого ОНБ:
    \[
        (A_e x)^T E (A_e y) = x^T y \quad \forall x, y
    \]
    ...
    Следовательно, матрица $A$ в ОНБ ортогональна.
\end{Proof}
...
\begin{theorem}
    ...
    \[
        A: E \to E.
    \]
    Тогда $A$ --- ортогональный л.о.
    \[
        \Longleftrightarrow
    \]
    ОНБ $e_1,\ldots,e_n$ переходит в ОНБ
    \[
        Ae_1,\ldots,Ae_n.
    \]
\end{theorem}
...
\begin{Proof}
    ...
    \[
        \{Ae_j\}_{j=1}^{n}
    \]
    состоит из ненулевых векторов и попарно ортогональна.
    ...
    Докажем:
    \[
        A \text{ --- ортогональный л.о.}
    \]

    Рассмотрим соответствие:
    \[
        x \mapsto (x_1,x_2,\ldots,x_n)^T
    \]
    в базисе
    \[
        e_1,\ldots,e_n.
    ...
    \[
        Ax
        =
        A(x_1e_1+\ldots+x_ne_n)
        =
        x_1Ae_1+\ldots+x_nAe_n.
    \]

    То есть координаты вектора $Ax$ в базисе
    \[
        Ae_1,\ldots,Ae_n
    \]
    равны
    ...
    то
    \[
        Ay=y_1Ae_1+\ldots+y_nAe_n.
    \]

    Найдём скалярные произведения.

    Так как $e_1,\ldots,e_n$ --- ОНБ:
    \[
        (x,y)=x_1y_1+\ldots+x_ny_n.
    \]

    Так как $Ae_1,\ldots,Ae_n$ --- ОНБ:
    \[
        (Ax,Ay)=x_1y_1+\ldots+x_ny_n.
    \]

    Следовательно,
    \[
        \forall x,y \in E\quad (Ax,Ay)=(x,y).
    \]

    Значит, $A$ --- ортогональный л.о. по определению.
\end{Proof}
\subsection{Сформулируйте теорему о спектральном разложении симметрической матрицы.}

\begin{theorem}[О спектральном разложении]
    $\forall$ симметрической матрицы $A$ $\exists$ такая ортогональная матрица $U$, что
    \[
        A = U\Lambda U^T,
    \]
    где
    \[
        ...
        \end{pmatrix}
    \]
    --- диагональная матрица с с.з. матрицы $A$, повторяющимися соответственно их алгебраической кратности.
\end{theorem}

\begin{Proof}
    $\forall$ симметрическая матрица является матрицей самосопряженного линейного оператора в ОНБ.
    
    Рассмотрим матрицу $A$ как матрицу самосопряженного линейного оператора в некотором ОНБ
    \[
        e = (e_1,\ldots,e_n).
    \]
    
    Для самосопряженного л.о. $\exists$ ОНБ из собственных векторов
    \[
        f = (f_1,\ldots,f_n),
    \]
    в котором его матрица диагональна.
    
    Тогда по формуле изменения матрицы линейного оператора при замене базиса:
    \[
        \Lambda = C_{e \to f}^{-1} A C_{e \to f}.
    \]
    
    Так как $C_{e \to f}$ --- матрица перехода между ОНБ, то она ортогональна.
    ...
    Умножим слева на $U$, а справа на $U^{-1}$:
    \[
        A = U\Lambda U^{-1}.
    \]
