...
\begin{remark}
    Производная по направлению максимальна в направлении градиента:
    \[
        \max_{\|\vec l\|=1}
        \frac{\partial f}{\partial l}(M)
        =
        \|\nabla f(M)\|.
    \]
    То есть наибольшее значение $\dfrac{\partial f}{\partial l}$ в точке $M$ равно длине градиента.
\end{remark}
...
\begin{theorem}[Критерий Сильвестра для экстремума]
    ...
    Если форма является знаконеопределенной (+ - +) или (0 - +), то точка не является точкой экстремума. Если какой-то из определителей равен 0, то критерий Сильвестра не даёт ответа. Тогда проводим дополнительное исследование: диагонализируем матрицу Гессе и/или смотрим поведение функции в окрестности точки.
\end{theorem}
