...
\begin{Proof}
    Докажем к примеру для неубывания по критерию Дарбу

    1) $f(x) \text{огр сверху} f(b) \text{и огр снизу } f(a)$\\
    2) Предположим противное:

    Пусть $I^* \neq I_*\implies 0 < I^* - I_* \leq S_\tau - s_\tau = \Sum{i = 1}{n}(M_i - m_i)|\D x_i|\leq d(\tau)\Sum{i = 1}{n}(M_i - m_i) = d(\tau)\Sum{i = 1}{n}(f(x_i) - f(x_{i-1})) = d(\tau)(f(b) - f(a))$\\
    $\D x_i = [x_{i-1}; x_i]$\\
    Возьмем $\tau$: $d(\tau) < \frac{I^* - I_*}{f(b) - f(a)}\cdot\frac{1}{2}$\\
    $0 < I^* - I_* \leq (I^* - I_*)\cdot\frac{1}{2}$\\
\end{Proof}

\begin{definition}[Опр1]
    $f(x) \text{ называется равн непр на E, если}$\\
    $\forall \eps > 0~ \exists\delta > 0 ~\forall x_1, x_2 \in E: |x_1 - x_2| < \delta$\\
    Верно $|f(x_1) - f(x_2)| < \eps$
\end{definition}

\begin{definition}[Опр2]
    $f(x)$ непр на E, если\\
    $\forall x_1 \in E~ \forall \eps > 0~ \exists\delta > 0 ~\forall x_2 \in E: |x_1 - x_2| < \delta$\\
    Верно $|f(x_1) - f(x_2)| < \eps$
