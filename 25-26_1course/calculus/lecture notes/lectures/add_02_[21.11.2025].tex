...
\begin{theorem}[Теорема о критерии замкнутости]
    \[
...
{\bf $\Rightarrow$} 
\[
    x_n \to A\ |x_n - A| \xrightarrow[n \to \infty ]{} 0
\]
о/п:
\[
...
о/п: $F$ не замекнуто $\Rightarrow \backslash F $ не открыто
\[
...
\end{Proof}
\begin{remark}
    Почему только $\R \text{ и } \varnothing$ одновременно открытые и пустые множества?
\end{remark}
\begin{example}
    \[
...
\end{definition}
\begin{theorem}[Теорема о критерии компактности]
...
\begin{exercise}
    \[
        \l\{\l[-\frac{1}{2^{2n} }; -\frac{1}{2^{2n+1}} \r]\r\} \bigcup^{\infty }_{n = 1} \{0\}
    \]
\end{exercise}
...
\begin{definition}
    ...
    \[
        \exists \eps > 0\ \exists \delta = \delta (\eps ) > 0\ \forall x_1, x_2 \in X\ (|x_1 - x_2| < \delta \Rightarrow |f(x_1) - f(x_2)| < \eps ) 
    \]
\end{definition}
\begin{example}
    \[
    ...
    \]
\end{example}
\begin{theorem}[Теорема о равномерной непрерывности (Кантора — Гейне)]
...
\begin{remark}
    $f$ равномерно непрерывна $\Leftrightarrow$ она непрерывна
\end{remark}
Функция Римана:
\[
    R(x) =
...
\begin{definition}
    ...
    \[
        \forall x, y \in X\ |f(x) - f(y)| \geq \mathcal{L} |x - y|
    \]
\end{definition}
